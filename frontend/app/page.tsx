...
export default function Home() {
      ...
      setMessages([
        {
          ...
          response: {
            ...
            analysis_plan: {
              ...
              selected_columns: [],
              column_selection_reason: "All columns were inspected during profiling.",
              planned_operations: ["Load file", "Infer schema", "Profile missing values and summary statistics"],
              output_type: "Text",
            ...
            execution: {
              status_steps: [
                ...
                { label: "Running Analysis", state: "complete" },
            ...
            confidence: { level: "High", reason: "Profile was generated directly from the uploaded dataset." }
          }
        }
      ]);
    } catch (err) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await sendChat(dataset.metadata.dataset_id, content, sessionId, educationalMode);
      setMessages((current) => [
        ...current,
        { id: crypto.randomUUID(), role: "assistant", content: response.assistant_message, response }
      ]);
    } catch (err) {
  ...
  return (
    <main className="flex h-screen overflow-hidden">
      <DatasetSidebar dataset={dataset} onUpload={handleUpload} uploading={uploading} error={error} />
        ...
        <header className="flex items-center justify-between border-b border-border bg-panel px-5 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted">{dataset ? "Grounded in uploaded data" : "Upload a dataset to begin"}</p>
            </div>
          </div>
          <label className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
            ...
            <input
              ...
              onChange={(event) => setEducationalMode(event.target.checked)}
          ...
          <div className="mx-auto max-w-5xl space-y-5">
            {messages.length === 0 ? (
              <div className="rounded-md border border-border bg-panel p-6 shadow-soft">
                <h2 className="text-lg font-semibold">Upload a dataset and start asking questions.</h2>
                <p className="mt-2 text-sm leading-6 text-muted">
                  Try asking about missing values, correlations, top categories, trends over time, or charts once profiling is complete.
                </p>
              </div>
            ) : null}
            {messages.map((message) => (
              <MessageCard key={message.id} message={message} showAnalysis={educationalMode} />
            ))}
            {thinking ? (
              <div className="rounded-md border border-border bg-panel p-4 text-sm text-muted shadow-soft">
                Building plan, generating code, and running local analysis...
              </div>
            ) : null}
          </div>
        </div>

        <form onSubmit={handleSubmit} className="border-t border-border bg-panel p-4">
