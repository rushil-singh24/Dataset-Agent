    ...
    <aside className="flex h-full w-full flex-col border-r border-border bg-panel lg:w-[360px]">
      <div className="border-b border-border p-5">
        ...
        <p className="mt-1 text-sm text-muted">
          Local analysis grounded in your uploaded dataset.
        </p>
      </div>
      ...
      <div className="space-y-5 overflow-y-auto p-5 scrollbar-thin">
        ...
        {uploading ? (
          <p className="text-sm text-muted">Profiling dataset...</p>
        ) : null}

        {error ? (
          <p className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-danger">
            {error}
          </p>
        ) : null}
        ...
        {readOnly ? (
          <p className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
            Dataset no longer loaded — upload again to ask new questions.
          </p>
        ) : null}
          ...
          <div className="space-y-5">
            <section className="rounded-md border border-border p-4">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <Metric
                  label="Rows"
                  value={dataset.metadata.row_count.toLocaleString()}
                />
                <Metric
                  label="Columns"
                  value={dataset.metadata.column_count.toLocaleString()}
                />
                <Metric
                  label="MB"
                  value={dataset.metadata.memory_usage_mb.toLocaleString()}
                />
              </div>
            </section>
                ...
                {dataset.columns.map((column) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="shrink-0 rounded bg-slate-100 px-2 py-1 text-[11px] text-muted">
                        {column.dtype}
                      </span>
                    </div>
